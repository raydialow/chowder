...
(define (make-sdl2-window-info)
  (define window-ptr (make-window-ptr))
  (define extcount-ptr (A-uint32))
  (cpointer-push-tag! extcount-ptr 'uint*)
  (SDL_Vulkan_GetInstanceExtensions window-ptr extcount-ptr #f)
  (define extnames-ptr (A*-string (ptr-ref extcount-ptr _uint32)))
  (SDL_Vulkan_GetInstanceExtensions window-ptr extcount-ptr extnames-ptr)
  (hash 'window window-ptr 'extcount extcount-ptr 'extnames extnames-ptr))

; gathers information on available validation layers (if vlayers are enabled)
;  returns an imm.hash containing appropo count of and names of validation layers.
;  >>> in production, validation layers should be left off. <<<
;  >>> in developent, validation layers can be helpful for debugging. <<<
(define (make-validation-info)
  (if (not (get-config "vk-validation"))
      (hash 'vlayers-count vkzero 'vlayers-names #f)
...
(define (make-inst-create-info)
  (let ([state (hash-union (hash-set (make-sdl2-window-info)
                                     'appinfo (make-application-info 0 0 0))
                           (make-validation-info))])
...
(define (make-instance)
  (let* ([vkinst-ptr (A-VkInstance)]
    ...
    (let ([vkresult-createinst  (vkCreateInstance (hash-ref state 'inst-create-info) #f vkinst-ptr)])
      (check-vkResult vkresult-createinst 'vkCreateInstance) 
